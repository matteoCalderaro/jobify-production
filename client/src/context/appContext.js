...
const AppProvider = ({ children }) => {
  ...
  const getJobs = async () => {
    const { page, searchStatus, searchJobType, sort, search } = state;
    ...
    try {
      ...
      const { jobs, numOfPages, totalJobs } = response.data;
      dispatch({
  ...
  const editJob = async () => {
    const { editJobId, position, company, jobLocation, status, jobType } =
      state;
